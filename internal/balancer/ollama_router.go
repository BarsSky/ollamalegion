...
func (or *OllamaRouter) Route(w http.ResponseWriter, r *http.Request) bool {
	...
	case "/api/embed":
		...
		return false
	case "/api/ps":
		// Round 21: don't proxy /api/ps to Ollama backend (which doesn't expose
		// this endpoint natively, fetchPS returns nothing). Fall through to
		// LlamaCppRouter which reads from llamaMetrics[id].LoadedModels
		// (Round 21 cppworker added real /api/ps handler too).
		return false
	case "/api/show":
		or.handleShow(w, r)
		return true
