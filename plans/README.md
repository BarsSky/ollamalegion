...
| **[Placement Policy — совместная работа режимов при 2+ бэкендах](multi-backend-placement-policy/PLAN.md)** | `plans/multi-backend-placement-policy/PLAN.md` | R66d (сентябрь 2026) | 🟡 **PROPOSAL**: per-model политика размещения (`single`/`pool`/`replicated`/`sharded`/`rpc` + `auto`) вместо глобального `operatingMode`; стандартная балансировка остаётся дефолтом, каждый нынешний режим выражается политикой. Открытый вопрос — транспорт раскладки (llama.cpp RPC vs B8.7) |
