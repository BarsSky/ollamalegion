...
| **[7.1 — GitHub Actions CI workflow](2026-q3-roadmap.md#7-cicd-и-тестирование)** | `.github/workflows/ci.yml` (4 jobs: test-self-hosted + test-ubuntu-fallback + lint + i18n) + `.golangci.yml` (8 линтеров) | Месяц 1 (июль 2026) | ✅ DONE 2026-06-28 (commit `64e100d`); push и фактический запуск CI отложены до получения PAT + установки 7.1a runner'а |
| [Production-ready (3.2, 3.3, B8.7, 7.1 → 7.4)](2026-q3-production-ready-plan.md) | `plans/2026-q3-production-ready-plan.md` | Sessions G+ (август-сентябрь 2026) | ⏳ WIP (7.1a ✅, 7.1 ✅; 7.4 ⏳) |
...
| 2026-06-28 | **Session 7.1a — Self-hosted Windows runner infrastructure** (`01b2afc`): `scripts/setup-runner.ps1` (config + register + service install через `actions/runner/`+`svc.sh`/`svc.ps1`), `scripts/check-runner.ps1` (status probe + auto-restart), `docs/ci/self-hosted-runner.md` (полная инструкция + `runs-on: [self-hosted, windows, ollamalegion-ci]`). Persistent GOMODCACHE (641 MB) + GOCACHE (3 GB) на этой машине. Установка runner'а отложена до получения GitHub PAT от пользователя. |
| 2026-06-28 | **Session 7.1 — GitHub Actions CI workflow + golangci-lint config** (`64e100d`): двустратегийный CI (primary self-hosted Windows + fallback `ubuntu-latest`). `.github/workflows/ci.yml` (190 LOC) с 4 jobs: `test-self-hosted` (build 4 бинарей, `go test -race -tags llama_stub -timeout 180s`, integration -short, coverage artifact), `test-ubuntu-fallback` (smoke build + `go test ./pkg/...` без CGO), `lint` (golangci-lint-action@v6 + v1.61.0, 8 линтеров, misspell US+RU, --only-new-issues), `i18n` (node 20 + `scripts/i18n_diff.js --strict`). Concurrency group + `cancel-in-progress: true`. `.golangci.yml` (75 LOC): 8 линтеров (govet/errcheck/staticcheck/ineffassign/unused/misspell/gosimple/typecheck), skip-dirs `c/llama.cpp`+`webui/node_modules`, skip-files `bridge_stub.go`+`bridge.c`+`*_gen.go`+`*_mock.go`. README бейджи обновлены: `[![CI](actions/workflows/ci.yml/badge.svg)]` + `[![Go Report Card](goreportcard.com/...)]`. Push и фактический запуск CI отложены до получения PAT + установки 7.1a runner'а. |
| **2026-Q3** | **Roadmap to 1.0 — Q3 W3-4 (Models tab gaps) ЗАКРЫТ полностью** (Sessions 13-19 + A-E) + **Session F ЗАКРЫТ ПОЛНОСТЬЮ** (UI/UX quick wins: F.0a/b/α/β/γ + F.4 — EOF diagnostics, SSE notifications, Health aggregator, /health page, i18n EN/RU баланс 1019=1019) + **CI/CD каркас ГОТОВ** (7.1a self-hosted runner + 7.1 GitHub Actions workflow). Production-ready фичи (rpc_coordinator, virtual_router, real ggml) запланированы в [production-ready plan](2026-q3-production-ready-plan.md) на август-сентябрь 2026. |
