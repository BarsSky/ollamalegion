...
| 7.1 | **GitHub Actions CI** — `go build`, `go test -race`, lint на каждый PR (`runs-on: [self-hosted, windows, ollamalegion-ci]` + fallback `ubuntu-latest` для случая когда runner оффлайн) | `.github/workflows/ci.yml` (новый, 4 jobs: test-self-hosted + test-ubuntu-fallback + lint + i18n), `.golangci.yml` (8 линтеров) | 0.5–1 день (после 7.1a) — ✅ DONE 2026-06-28 (commit `64e100d`); push и фактический запуск CI отложены до получения PAT + установки 7.1a runner'а |
