...
func TestTranslateUsageChunkToOllama_GeneratePath(t *testing.T) {
	...
	result := translateOpenAISSEDataToOllama("/api/generate", usageChunk, "qwen3", nil, time.Time{}, time.Time{}, time.Time{})
	if result == nil {
...
func TestTranslateUsageChunkToOllama_ContentChunkRegression(t *testing.T) {
	...
	result := translateOpenAISSEDataToOllama("/api/chat", contentChunk, "gemma-4", nil, time.Time{}, time.Time{}, time.Time{})
	if result == nil {
