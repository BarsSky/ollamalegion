...
func TestWireFormat_ChatStreaming_FullSchema(t *testing.T) {
	...
	for _, raw := range chunks {
		result := translateOpenAISSEDataToOllama("/api/chat", []byte(raw), "test-model", &seenReasoning, streamStart)
		if result == nil {
			continue
		}
...
func TestWireFormat_ChatStreaming_ToolCallsSchema(t *testing.T) {
	...
	var ndjsonChunks []map[string]interface{}
	for _, raw := range chunks {
		result := translateOpenAISSEDataToOllama("/api/chat", []byte(raw), "test-model", &seenReasoning, time.Time{})
		if result == nil {
			continue
		}
		var c map[string]interface{}
...
func TestWireFormat_GenerateStreaming_FullSchema(t *testing.T) {
	...
	var ndjsonChunks []map[string]interface{}
	for _, raw := range chunks {
		result := translateOpenAISSEDataToOllama("/api/generate", []byte(raw), "test-model", &seenReasoning, time.Time{})
		if result == nil {
			continue
		}
		var c map[string]interface{}
	...
	for _, sc := range scenarios {
		t.Run(sc.name, func(t *testing.T) {
			...
			for _, raw := range sc.chunks {
				result := translateOpenAISSEDataToOllama(sc.path, []byte(raw), "test-model", &seenReasoning, time.Time{})
				if result == nil {
					continue
				}
				var c map[string]interface{}
