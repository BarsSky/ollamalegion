...
**Факт по R76 (2026-09-24, `internal/balancer/placement_reject_r76.go`,
`placement_fit_r75.go`):**
...
* ✅ `auto.requireHomogeneous`: набор бэкендов сужается до группы с одинаковой
  ...
  `reason`;
* ✅ `degraded` уже отдаётся в решении (`/api/v1/placement`, R75) и в логе WARN;
* ⏳ осталось: WebUI-колонка «Размещение» (вкладка моделей + страница «Режимы»),
  автопересборка раскладки при изменении состава бэкендов, `degraded` в общих
  метриках (`/api/v1/metrics`).
