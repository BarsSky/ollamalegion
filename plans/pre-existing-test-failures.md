...
| PF-4 | `TestOpenAIChat_SlowFirstToken_HoldsConnection` | `tests/first_byte_timeout_test.go:114` | ✅ FIXED 2026-06-27 (Session 18 — принудительное закрытие `Listener.Close()` + `CloseClientConnections()` в teardown, паттерн из `TestOpenAIChat_HeaderTimeout_StillWorks`. Также defensive: `<-r.Context().Done()` в keepalive-цикл `startSlowSSEServer` для немедленного выхода при отмене клиента) |
