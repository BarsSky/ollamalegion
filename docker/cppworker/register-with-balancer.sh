...
# Cross-sync env: Go side reads CPPWORKER_BALANCER_URL/CPPWORKER_BALANCER_TOKEN
# (cmd/cppworker/balancer_register.go). This shell script historically uses
# BALANCER_URL/BALANCER_API_TOKEN. Accept both, prefer CPPWORKER_*.
...
# Default 18092 matches the GPU HTTP port (cmd/cppworker/main.go: --port default
# is 18092; the legacy CPU port 18091 must be set explicitly via CPPWORKER_PORT).
