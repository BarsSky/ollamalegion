...
* llama.cpp got **first-class tensor parallelism** via "meta
  backend" (PR #19378, ~April 2026) behind the `--split-mode tensor` flag.
* Default mode **`--split-mode layer`** (pipeline parallel) is already
  production-ready, works on top of the `tensor_split` config.
* Our current `StubTPRuntime` (internal/rptensor/tp_runtime.go) is a pure
  stub and does not call real llama.cpp.
* The C bridge (c/bridge/bridge.h) already has a `tensor_split` config, but
  does not pass `split_mode` into `llama_context_params`.
* Real integration requires: 1) C bridge functions for
  multi-GPU split, 2) wiring into `RealTPRuntime` (Go side), 3) build
  matrix with `-DGGML_CUDA_NCCL=ON`, 4) CI multi-GPU runner.
* **Recommendation:** for the 1.0 release we stay on `StubTPRuntime`.
  Layer-mode TP (pipeline parallel) goes to P.4 (post-1.0 enhancement).
  True tensor split goes to P.5 (research track, requires CUDA/NCCL team).

---

## 1. llama.cpp TP API Survey

### 1.1 Split modes (3 modes)
...
```bash
# Default (NCCL is enabled by default for CUDA):
cmake -DGGML_CUDA=ON ...

# Manual NCCL enable (default in recent versions):
...
Must install manually (or check the CMake log for `NCCL found`).
...
* Workers run on individual GPUs; TP coordination would happen at
  the worker level (multi-GPU per worker) OR coordinator level (multi-worker
...
* **Pipeline parallelism (P.1, rpc_coordinator mode)** — coordinator
  ...
  This is equivalent to llama.cpp `--split-mode layer` at the cluster
...
**Goal:** worker on a multi-GPU box uses `tensor_split` for layer
distribution. No NCCL needed (layer mode uses CUDA streams only).

**Changes:**
1. `c/bridge/bridge.c`: in `LoadModel`, pass `tensor_split` array to
   `llama_context_params`. Verify that the existing `tensor_split` field
   works (currently may be silently ignored).
2. `internal/rptensor/`: new `LayerTPRuntime` that calls
   `bridge.LoadModel(tensor_split=[0.5, 0.5])` for a 2-GPU host.
   Returns success/failure to coordinator.
3. `cmd/cppworker/main.go`: parse `LB_TENSOR_SPLIT=0.5,0.5` environment variable,
   pass to bridge.
...
**Goal:** worker on a 2-GPU box uses tensor parallelism for dense models.

**Changes (on top of 3.1):**
1. C bridge: add `split_mode` parameter to `ModelConfig` + `LoadModel`.
2. Verify llama.cpp PR #19378 API in current upstream (may need a patch
   if the API changed).
...
- Document `tensor_split` environment variable support in worker (3.1) as a
...
- **P.6: RealTPRuntime in rptensor** (3.3, 10-15 days, research-grade)
  — replaces stub, but only valuable if we have a CUDA/NCCL team.
...
1. **Hardware:** Do we have 2+ GPU boxes available for development?
   ...
   a multi-GPU dev box or cloud instance.)
