...
**Статус:** P0 (R72), P1 (R74), P1.5 (R75), P3 (R76–R78) РЕАЛИЗОВАНЫ.
Открыт P2 (`sharded`/`rpc` на реальном транспорте — отложен решением пользователя:
нужен выбор llama.cpp RPC vs собственный B8.7) и P4 (нагрузочные KPI).
**Автор:** сессия R66d
...
### P3 — Отказоустойчивость и UX (≈1 неделя) — ✅ СДЕЛАНО (R76, R77, R78, R79)
...
**Факт по R76/R77/R78 (2026-09-24):**
...
* ✅ `degraded` в общих метриках (R78): `GET /api/v1/metrics` → блок `placement`
  (`configured/enabled/fallback/operatingMode`, `byStrategy`, `degraded`,
  `notExecutable`, `warnings`, `replicationReady/Groups`);
* ✅ автопересборка раскладки (R78): `AddBackend`/`RemoveBackend` →
  `schedulePlacementResync` → `syncPlacementReplicationGroups` + `EnsureInstances`
  (асинхронно: callbacks менеджера берут `p.mu.RLock`, а в registry-методах
  `p.mu` уже удерживается) — новый бэкенд получает реплику сразу;
* ✅ legacy `QueueManager` выведен из эксплуатации (R78): канал, пул worker'ов и
  `dispatchRequest` удалены, остались статистика/история; `current_size` в
  `/api/v1/queue/stats` теперь показывает ожидающих **единой** очереди;
* ✅ хвост §6 для **явных** стратегий (R79): R76 проверял только `auto` и
  «неисполнимые в сборке» `sharded`/`rpc`. Явное `strategy: replicated` при одном
  подходящем бэкенде создавало группу с `minInstances=1` и обслуживалось одной
  копией, а `X-LB-Placement` рапортовал `replicated`. Теперь решение по правилу
  модели/класса проверяется тем же VRAM-fit, что и `auto`, и при нехватке копий
  помечается `degraded` с числами (`подходящих бэкендов 1, нужно 2`) → 503 при
  `fallback=error`, `X-LB-Placement: single` + `X-LB-Placement-Fallback:
  replicated` при `fallback=single`, `degraded` в метриках при
  `auto.allowDegraded=true`. Глобальный `operatingMode` и per-request override
  не проверяются (обратная совместимость); явный `replicated` без
  `auto.minBackends` поднимает группу до двух инстансов.

### P4 — Нагрузочные KPI (2-3 дня) — ✅ СДЕЛАНО (R80)
...
**Факт по R80 (2026-09-25):** замеры на throwaway-стенде (2×cppworker на одной
RTX 3070 8 ГБ, модель 2.33 ГБ, ctx 4096) — полный отчёт с таблицами и
методикой: `plans/2026-09-25-placement-p4-load-kpi.md`. Кратко:

* `single` (1 копия, `n_parallel=1`): 38.6 tok/s на запрос; 4 параллельных →
  33.5 tok/s aggregate, ожидание в очереди до 11.8 с; VRAM 4.6 ГБ;
* `replicated` (2 копии): 32.8 tok/s на запрос; 4 параллельных → 25–34 tok/s
  aggregate (в пределах шума), ожидание до 8.1 с (два слота); VRAM 7.6 ГБ;
* `n_parallel=4` + `batched_scheduler`: 34.6 tok/s на запрос; 4 параллельных →
  34.1 tok/s aggregate, admission-очередь не задействована (заголовков
  `X-Queue-*` нет — ожидание уезжает внутрь cppworker);
* failover реплики: 2 из 3 запросов во время падения обслужила живая копия,
  503 на стратегию нет; восстановление копии 21.6 с (модель в VRAM), группа
  снова `loaded=2` через 31.7 с;
* вывод: на одной GPU `replicated` даёт слоты очереди и отказоустойчивость, но
  не пропускную способность (33–34 tok/s упирается в GPU); цена — +3.2 ГБ VRAM
  на копию. `sharded` в KPI не измерялся: транспорт раскладки — этап P2.
* стенд вскрыл и закрыл три дефекта: R79 (явный `replicated` исполнялся одной
  копией), R80 (группа не видела загруженные копии + VRAM-fit отбивал 503, когда
  копии уже в VRAM) и R80-HA (§6: падение одной реплики не повод отказывать).
  Открытые пункты (агент не должен владеть health-статусом, строгий обход реплик,
  событийное `LOADING→LOADED`) — в конце отчёта P4.
