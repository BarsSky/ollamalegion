...
## [0.5.40 — Round 82 (2026-09-25)]

### 🐛 Failover запроса, выбранного на упавшую реплику: 120 с ожидания → 200 за 3 с

Последний хвост P4. Воспроизведение (стенд 2×cppworker): `docker stop` одной
реплики, затем запросы сразу после падения. Запрос, который ротация выбрала на
умершую копию, вёл себя так:

* async-загрузка модели падала за **30 мс** (DNS: `no such host`), но
  `waitForModelLoad` этого не видел и ждал полный `LB_AUTO_LOAD_WAIT_SEC`
  (в стенде **300 с**), клиент отваливался по своему таймауту (120 с);
* живая копия модели на другом бэкенде всё это время была доступна.

Три исправления:

1. **Сигнал о провале async-загрузки.** Горутина загрузки теперь сообщает
   результат ожидающему через канал; `waitForModelLoad` падает сразу (раньше
   причину видел только лог), а при истечении окна отдаёт actionable текст
   (`model load failed: ... no such host`) вместо «подожди 30-180 с».
2. **Выход из ожидания, если бэкенд выпал.** `waitForModelLoad` проверяет
   `backendUnavailable` (не healthy / нет в пуле) и падает сразу.
3. **Переезд на живую копию.** Новый `ensureModelLoadedWithFailover` подключён ко
   всем пяти inference-путям llama.cpp (`/v1/chat/completions`, `/v1/completions`,
   `/v1/embeddings`, `/api/chat`, `/api/generate`):
   * бэкенд недоступен или ошибка транспортного уровня → пробуем до 3 других
     копий/узлов;
   * загрузка не удалась, но рядом есть **готовая** копия → обслуживаем с неё без
     ожидания;
   * при переезде обновляются `X-Backend-ID` и снапшот `state` (иначе URL
     апстрима оставался от старого бэкенда).

**Живая проверка (R82, тот же сценарий):** 4 запроса сразу после `docker stop`
одной реплики → **4×HTTP 200**, все обслужены живой копией (`X-Backend-ID: p4-a`),
время 0.5–3.7 с (запросы, выбравшие мёртвую копию, переехали за ~3.7 с). Контрольный
замер: во время падения 2 из 2 запросов → 200 (cpp-a обработал 2 запроса), 0
неудач и 0 зависаний; восстановление копии — модель в VRAM через 23 с, группа
`loaded=2` через 33 с.

**Тесты:** `internal/balancer/failover_r82_test.go` (5: мгновенный выход при
провале загрузки; выход при недоступном бэкенде; переезд на живую копию;
...
Четыре дефекта, найденных нагрузочным стендом P4 (отчёт
`plans/2026-09-25-placement-p4-load-kpi.md`), закрыты.

**1. Heartbeat агента больше не владеет health-статусом бэкенда**

`POST /api/v1/agents/heartbeat` (и heartbeat по `backendID`) безусловно звали
`UpdateBackendStatus(..., StatusHealthy)`. Агент — отдельный контейнер: когда
cppworker за ним мёртв, агент продолжает слать heartbeat и возвращал бэкенд в
пул, пока health-check помечал его недоступным. Итог замера: 1 запрос из 3
уходил на мёртвую копию и висел до клиентского таймаута (120 с). Теперь
статусом владеет только health-check (`internal/balancer/health.go`), а агент
отмечает факт контакта (`HasAgent`/`LastAgentContact`).

**2. Группа репликации видит загруженные копии во всех источниках метрик**

`backendHasModelByID` смотрел только `metrics[id]` (агентские `RunningModels`/
`LlamaCpp.LoadedModels`), а поллер cppworker пишет в отдельную карту
`llamaMetrics` и может отдать модель без `context_length`. В живом прогоне это
дало «две копии в VRAM, группа показывает `loading=2/loaded=0`»: группа
пыталась загрузить то, что уже загружено. Теперь проверяются все источники
(включая объединённый `metrics[id].Models`), а лишнее требование
`ContextLength > 0` убрано. Живая проверка: контроллер группы усыновляет копии
на ближайшем тике — `group instances adopted from backends: [p4-b, p4-a]`.

**3. Строгий обход реплик и подключение группы к inference-пути llama.cpp**

`selectInstance` выбирал «наименее загруженный» инстанс, а при равной загрузке
(обычный случай) результат зависел от порядка обхода map. Теперь порядок
детерминированный: меньшая загрузка → меньший `UseCount` → меньший ID, а
выбранный инстанс сразу учитывается (`UseCount++`). Главное: inference-путь
llama.cpp группу вообще не спрашивал — он звал `findModelOnLlamaCppBackend`,
который возвращает ПЕРВЫЙ бэкенд с моделью из обхода map. Добавлен
`selectLlamaCppBackendForModel`: группа репликации → бэкенд с загруженной
моделью → любой healthy. Живой замер на двух копиях: распределение **4:4**
(было 7:1 и 8:4), 4 параллельных запроса — aggregate **42.5 tok/s** при 0
ожиданий в admission-очереди.

**4. Событийное подтверждение/снятие реплики**

`POST /api/v1/internal/llama-model-loaded` теперь сразу повышает инстанс группы
`LOADING → LOADED` (`AdoptLoadedInstance`), а `llama-model-unloaded` — убирает
его (`DropInstance`) и дозагружает копию, не дожидаясь 30-секундного тика
поллера метрик.

**Тесты:** `internal/modelreplication/model_group_r81_test.go` (5: событийное
усыновление, создание инстанса по событию, снятие по выгрузке, чередование
a/b/a, приоритет наименее загруженного), `internal/api/agent_health_r81_test.go`
(3: heartbeat по X-Agent-ID и по backendID не меняет статус, но отмечает
контакт; событие `llama-model-loaded` подтверждает реплику, `unloaded` снимает),
обновлён `tests/integration_test.go` (шаг «Send Heartbeat» теперь проверяет новый
контракт). Регрессии: `./internal/... -race`, `./cmd/...`, `./tests/... -short`
— зелёные.

**Открытый пункт (R82):** запрос, который выбрал реплику прямо перед её
падением, ждёт авто-загрузку модели до `LB_AUTO_LOAD_WAIT_SEC` (в замере — 120 с
до клиентского таймаута), хотя живая копия с той же моделью есть. Нужен failover
на другую копию (повторный выбор бэкенда после connection-level ошибки) или
ранний выход из ожидания загрузки, если бэкенд выпал из пула.

## [0.5.38 — Round 80 (2026-09-25)]

### 🐛 Репликация видит реальность + нагрузочные KPI P4 (этап P4 закрыт)

Нагрузочный стенд (2×cppworker на одной RTX 3070, стратегия `replicated`)
вскрыл три дефекта — они закрыты в этом же раунде; полный отчёт с таблицами:
`plans/2026-09-25-placement-p4-load-kpi.md`.

**1. Группа репликации усыновляет уже загруженные копии**

`syncPlacementReplicationGroups` знал только те копии, которые загрузил сам
менеджер. После рестарта балансера при двух реально загруженных копиях группа
была пуста (`total: 0`), контроллер грузил ЛИШНЮЮ третью копию на «свободный»
бэкенд, а трафик `replicated` уходил в одну копию (по счётчикам cppworker 7:1
при двух равных репликах). Теперь у менеджера есть callback
`SetLoadedBackendsFn` («где модель реально загружена» — по метрикам
cppworker/агента): `syncLoadedInstances` усыновляет готовые копии, сбрасывает
протухшие инстансы (модели нет и тёплое окно `loadedGracePeriod=60 с` прошло,
`LOADING` старше `loadingTimeout=15 мин`) и повышает `LOADING → LOADED` только
после подтверждения метриками.

**2. Инстанс группы больше не выдаёт «загрузку запрошена» за «модель загружена»**

`scaleUpGroup` помечал инстанс `LOADED` сразу после вызова warmup, хотя загрузка
асинхронная (cppworker грузит модель десятки секунд). Группа рапортовала «2
копии готовы», когда моделей в VRAM не было, и не перезагружала их. Теперь
инстанс остаётся `LOADING`, `ensureGroupInstances` считает запрошенные загрузки
(`inFlight`) и не запускает дублирующие.

**3. VRAM-fit считает «модель уже загружена» по снапшоту метрик**

`getModelLoadedCtxFromMetrics` смотрел только `llamaMetrics` поллера и требовал
`context_length > 0`. Когда обе копии реплики уже загружены и свободного VRAM
почти нет, фит-проверка (R75/R79) объявляла «подходящих бэкендов 0» и при
`fallback=error` отклоняла **все** запросы 503-й, хотя модель лежала в VRAM.
Теперь используется общий путь `backendHasModel` (Ollama `RunningModels` +
llama.cpp `LoadedModels`).

**4. §6: падение одной реплики — не повод отказывать**

Строка §6 «один из бэкендов реплик unhealthy»: при `fallback=error` запросы
больше не отбиваются 503-й, пока жива хотя бы одна копия — обслуживаем с неё, а
снижение избыточности видно в причине («готовых копий 1 из 2») и в статистике
группы. 503 остаётся для случая «копий нет вообще» (стратегию исполнить нельзя).

**P4 — нагрузочные KPI (стенд 2×cppworker, модель 2.33 ГБ, ctx 4096, RTX 3070 8 ГБ):**

| Конфигурация | 1 запрос | 4 параллельных (aggregate) | Очередь | VRAM |
|---|---|---|---|---|
| `single` (1 копия, `n_parallel=1`) | 38.6 tok/s | 33.5 tok/s | позиции 1/2/3, до 11.8 с | 4.6 ГБ |
| `replicated` (2 копии) | 32.8 tok/s | 25–34 tok/s | до 8.1 с (2 слота) | 7.6 ГБ |
| `n_parallel=4` + `batched_scheduler` | 34.6 tok/s | 34.1 tok/s | заголовков `X-Queue-*` нет | 4.4 ГБ |
| failover (stop одной копии) | — | 2 из 3 запросов → 200 с живой копии, 503 нет | — | восстановление 21.6–31.7 с |

Вывод: на одной GPU `replicated` даёт слоты очереди и отказоустойчивость, но не
пропускную способность (33–34 tok/s упирается в GPU); цена — +3.2 ГБ VRAM на
копию. `n_parallel=4` убирает видимую очередь, но то же ожидание уезжает внутрь
cppworker. Открытые пункты (в отчёте P4): агент не должен владеть health-статусом
бэкенда (в замере heartbeat «воскресил» мёртвую копию → 1 запрос из 3 завис до
клиентского таймаута 120 с), строгий обход реплик вместо «наименее загруженного»
с случайным порядком, событийное `LOADING → LOADED` вместо опроса метрик.

**Тесты:** `internal/modelreplication/model_group_r80_test.go` (5),
`internal/balancer/replication_reality_r80_test.go` (4), правка
`queue_retire_r78_test.go` (`minBackends=2` вместо `1` — прежний вариант проходил
только за счёт гонки на `LOADING`). Регрессии: `./internal/... -race` — зелёный.
...
#### Более мощное железо, ожидание вместо 503, офлайн-ассеты WebUI

Пользователь сообщил (2026-09-23, стек на более мощном GPU): «нет возможности
регулировать максимально доступное VRAM — оно почему-то имеет ограничение жёстко
в 32768, даже если видеопамяти с запасом»; «если модель не загружена, клиенту
сразу прилетает ошибка, что будет она загружена через 30-180 секунд, и только
повторный запрос от клиента проходит»; «балансер резко отбивает повторный запрос
от клиента, выдавая 503, а не поставляя в очередь — скорей всего причина в том,
что не различает разных пользователей клиента OpenWebUI, который может
предоставлять модель сразу нескольким пользователям одновременно»; «даже если от
одного пользователя идёт новый запрос, балансер его не ставит в очередь на
отработку и не делает как новую сессию»; «нужны статические ассеты, выкаченные в
образ WebUI, чтобы не терять стили и иконки на машинах без сети».

**R67a — потолок контекста, авто-загрузка, офлайн-ассеты:**

1. **«VRAM жёстко ограничен 32768».** Потолок n_ctx считался без учёта типа
   KV-cache и с фиксированным резервом VRAM, поэтому «лишняя» видеопамять не
   использовалась. Добавлен расчёт по типу KV-кэша (`f16`/`q8_0`/`q4_0`),
   конфигурируемый резерв `CPPWORKER_VRAM_OVERHEAD_MB` (default 2048, на 24 GB
   можно уменьшить) и повторное чтение свободной VRAM перед расчётом
   (`refreshGPUDeviceVRAM`). Замер A/B на одном GGUF (RTX 3070 8 GB, ctx 8192):
   `max_vram_n_ctx` 69 893 (f16) → 248 510 (q4_0) = ×3.56, `max_ram_n_ctx`
   233 016 → 262 144. Потолок `LB_NCTX_RELOAD_MAX_N_CTX` (default 131072) задаёт
   оператор, фактический предел считает cppworker по VRAM.
2. **«Ошибка сразу, проходит только повторный запрос».** В async-режиме
   авто-загрузки балансер запускал загрузку в горутине и НЕМЕДЛЕННО отвечал
   503 «model X load started, waiting ~30-180s»; клиент (OpenWebUI) показывал
   ошибку, и запрос проходил только со второй попытки. Добавлено ожидание
   `LB_AUTO_LOAD_WAIT_SEC` (default 180, 0 = прежнее поведение, отрицательное —
   до 30 мин): состояние опрашивается напрямую у cppworker
   (`/api/models/load/progress`), первый же запрос обслуживается. Проверено на
   живом стеке: незагруженная модель + один чат → **HTTP 200 за 87.0 с** без
   повторного запроса. Если cppworker сообщает об ошибке загрузки, клиент
   получает её текст вместо общего 503.
3. **Офлайн-ассеты WebUI.** Сборка образа больше не тянет пакеты из сети
   (`apk add gettext` убран), nginx.conf генерируется busybox-sed, добавлен
   `scripts/check_webui_assets.py` (CDN + отсутствующие ассеты) в CI-джобу
   `webui-static-checks`. Проверено: 66/66 ассетов отдаются 200, «89 локальных
   ассетов в HTML + 20 url() в CSS».
4. **Частичное обновление профиля модели из WebUI сбрасывало поля.**
   PUT с одним полем затирал остальные (`contextLengthAuto`, `contextLengthMax`,
   таймауты, kv/fa/mmap). Добавлен `profilePresence`: обновляются только
   переданные поля, `null` = сброс, новый профиль по-прежнему требует
   `contextLength`.

**R67b — admission-очередь и per-user сессии (пункты 3 и 4 жалобы):**

5. **«Резко отбивает повторный запрос 503 вместо очереди».** Inference-хендлеры
   llama.cpp (`/api/chat`, `/api/generate`, `/v1/chat/completions`) выбирали
   бэкенд и проксировали запрос, НЕ беря слот: `maxConcurrentReqs` на этот путь
   не влиял, при трёх параллельных генерациях `activeRequests` оставался 0.
   Добавлена admission-очередь (`internal/balancer/admission_queue.go`): занятые
   слоты → запрос ждёт `LB_ADMISSION_WAIT_SEC` (default 300, 0 = прежнее
   поведение), после освобождения слота обслуживается ПЕРВЫМ же запросом.
   503 остаётся последним рубежом и теперь несёт `Retry-After`,
   `X-Queue-Position`, `X-Queue-Wait-Ms`, а ожидающие видны в
   `GET /api/v1/queue/stats` (поле `admission`).
6. **«Не различает разных пользователей OpenWebUI».** Все пользователи
   OpenWebUI приходят с одного IP, с одним User-Agent и ОДНИМ
   Authorization-токеном, поэтому fingerprint совпадал: на живом стенде три
   запроса от трёх разных пользователей (`X-User-Id`) дали одну сессию с
   `requestCount=3`. Теперь идентичность берётся из `X-User-Id` /
   `X-OpenWebUI-User-Id` / `X-User-Email`, а также из тела запроса
   (`user`/`user_id`) и вложенного `metadata` (`user_id`, `chat_id`,
   `session_id` — так помечает запросы OpenWebUI). Разные пользователи и разные
   чаты одного пользователя → разные сессии.
7. **«Новый запрос того же пользователя не ставится в очередь и не делается
   новой сессией».** Новая сессия/чат получает собственную позицию в очереди, а
   повторные запросы уже обслуживаемой сессии уступают сессиям, которые ещё не
   обслуживались (окно справедливости 30 с): один пользователь не занимает
   очередь целиком.
8. **Учёт реального параллелизма.** Активные запросы llama.cpp теперь проходят
   через слоты балансера, поэтому `activeRequests` бэкенда в WebUI/API
   соответствует реальной нагрузке (раньше показывал 0 под нагрузкой).

Проверено локально:
`go test -tags llama_stub -run R67b ./internal/balancer/` — 6 юнит/HTTP-тестов
(ожидание слота, выключенное ожидание, таймаут с заголовками, приоритет новой
сессии, источники ключа сессии, разделение пользователей, HTTP-уровень
`maxConcurrentReqs=1`), плюс регрессионный набор `./tests/...`.
...
Ещё три дефекта, найденные при проверке в реальном браузере:

10. **Ложная «Ошибка загрузки» при клике «Загрузить модель».** Карточка
    локального файла отдаёт имя С расширением (`X.gguf`), cppworker
    регистрирует модель БЕЗ него → балансер 5 поллов подряд не находил модель
    и рапортовал `auto-load failed: model "X.gguf" did not appear in cppworker
    model list or load progress` при фактически загруженной модели (в UI
    одновременно «Ошибка загрузки» и карточка загруженной модели).
    Добавлен `modelNameMatches()` (регистр/.gguf/путь/частичное имя) и применён
    во всех местах сравнения имён одной модели (поллинг загрузки,
    `IsLlamaCppModelLoaded`, `checkLlamaCppModelLoaded`, метрики, autotune,
    оценка таймаутов, X-Model-* заголовки); WebUI отправляет каноническое имя.
11. **Reload с новыми настройками оставлял модель частично на CPU.**
    `reloadAllLoadedWithDefaults` считал число GPU-слоёв ДО выгрузки старой
    копии → оценка видела занятую VRAM и занижала результат (34/42 и 27/42 при
    последующем «VRAM sufficient, vramFreeMB=7069, нужно 6537»). Расчёт
    перенесён после `UnloadModel` → reload снова даёт 42/42 (проверено).
12. **`cmd/cppworker/handlers` — мёртвая копия `handlers_config.go`** без
    расширения `.go` (отслеживалась в git, toolchain её не компилирует, но
    выглядит как исходник и содержит устаревшую сигнатуру). Удалена.

Плюс CI: job `Lint (golangci-lint)` падал на `fieldalignment` для новых полей
`backendModelOpRequest` / `clusterReloadModelRequest` — порядок полей приведён к
оптимальному, линтер зелёный.
